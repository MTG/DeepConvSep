...
function Bach10_eval_only(dataset_folder,base_estimates_directory,output_dir,id)
...
sources_names_files = {'bassoon','clarinet','saxphone','violin'};
sources_names = {'bassoon','clarinet','saxphone','violin'};
...
%for imethod_name = 1:length(methods_folders )
    ...
    %for subset_index = 1:2
        sources_folder = fullfile(dataset_folder,'Sources');
        ...
        %if ~isequal(exist( fullfile(output_dir,method_name,[song_name '_results.mat']), 'file'),2) %ADDED CODE
            ...
            for source_index = 1:4
                source_file = fullfile(sources_folder,song_name,[song_name '-' sources_names{source_index},'.wav']); %ADDED CODE
        ...
        if ~isequal(exist(fullfile(output_dir,method_name), 'dir'),7) 
            ...
            song_name = songs_list(song_index+2).name;
            results_file = fullfile(output_dir,method_name,[song_name '_results.mat']);
