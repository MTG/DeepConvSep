...
def train_auto(fun,train,transform,testdir,outdir,num_epochs_mse=30,num_epochs_ILD=10,model="1.pkl",scale_factor=0.3,load=False,
    skip_train_mse=False,skip_train_ILD=False,skip_sep=False, chunk_size=60,chunk_overlap=2,nsamples=40,batch_size=32,
    batch_memory=50, time_context=30, overlap=25, nprocs=4,mult_factor_in=0.3,mult_factor_out=0.3,mix_type='mixture'):
    """
    ...
    if not skip_train_mse:
        ...
        for epoch in range(num_epochs_mse):
            ...
            logging.info("Epoch {} of {} took {:.3f}s".format(
                epoch + 1, num_epochs_mse, time.time() - start_time))
...
#NEW ILD TRAINING---------------------------------------------------------
    ...
    if not skip_train_ILD:
        ...
        for epoch in range(num_epochs_ILD):
            ...
            logging.info("Epoch {} of {} took {:.3f}s".format(epoch + 1, num_epochs_ILD, time.time() - start_time))
            logging.info("  training loss:\t\t{:.6f}".format(train_err/train_batches))
    ...
    if not skip_train_mse:
        ...
        for epoch in range(num_epochs_mse):
            ...
            logging.info("Epoch {} of {} took {:.3f}s".format(epoch + 1, num_epochs_mse, time.time() - start_time))
            logging.info("  training loss:\t\t{:.6f}".format(train_err/train_batches))
            for j in range(nchannels):
                for i in range(nsources):
                    logging.info("  training loss for "+sources[i]+" in mic "+str(j)+":\t\t{:.6f}".format(errs[j][i]/train_batches))
    ...
    if not skip_sep:
        ...
        for sub in subsets:
            for d in sorted(os.listdir(os.path.join(db,'Mixtures',sub))):
                if not d.startswith('.'):
                    print os.path.join(db,'Mixtures',sub,d,mix_type+'.wav')
                    audio, sampleRate, bitrate = util.readAudioScipy(os.path.join(db,'Mixtures',sub,d,mix_type+'.wav'))
...
if __name__ == "__main__":
    ...
    if len(sys.argv)>-1:
        ...
        climate.add_arg('--nepochs_mse', help="number of epochs to train the net using MSE condition")
        climate.add_arg('--nepochs_ILD', help="number of epochs to train the net using ILD condition")
        ...
        climate.add_arg('--skip_train_mse', help="skip mse training")
        climate.add_arg('--skip_train_ILD', help="skip ILD training")
        climate.add_arg('--binaural', help="indicate if the audio is a binaural mixture or not")
        climate.add_arg('--skip_sep', help="skip separation")
        ...
        if kwargs.__getattribute__('nepochs_mse'):
            nepochs_mse = int(kwargs.__getattribute__('nepochs_mse'))
        else:
            nepochs_mse = 30
        if kwargs.__getattribute__('nepochs_ILD'):
            nepochs_ILD = int(kwargs.__getattribute__('nepochs_ILD'))
        else:
            nepochs_ILD = 10
        ...
        if kwargs.__getattribute__('load'):
            ...
            load = False
        if kwargs.__getattribute__('skip_train_mse'):
            skip_train_mse = int(kwargs.__getattribute__('skip_train_mse'))
        else:
            skip_train_mse = False
        if kwargs.__getattribute__('skip_train_ILD'):
            skip_train_ILD = int(kwargs.__getattribute__('skip_train_ILD'))
        else:
            skip_train_ILD = False
        if kwargs.__getattribute__('skip_sep'):
            skip_sep = int(kwargs.__getattribute__('skip_sep'))
        else:
            skip_sep = False
        if kwargs.__getattribute__('binaural'):
            binaural = int(kwargs.__getattribute__('binaural'))
        else:
            binaural = False
    ...
    if not load:
        model_name = model + '_mseEp=' + str(abs(skip_train_mse-1)*nepochs_mse) + '_ILDEp=' + str(abs(skip_train_ILD-1)*nepochs_ILD)
    else:
        model_name=model
    ...
    if not os.path.exists(os.path.join(output,'output',model_name)):
        os.makedirs(os.path.join(output,'output',model_name))
    if not os.path.exists(os.path.join(output,'models')):
        os.makedirs(os.path.join(output,'models'))

    train_errs=train_auto(fun=funcs[function],train=ld1,transform=tt,outdir=os.path.join(output,'output',model_name),
        testdir=db,chunk_size=chunk_size,chunk_overlap=chunk_overlap,
        model=os.path.join(output,"models",model_name+".pkl"),num_epochs_mse=nepochs_mse,num_epochs_ILD=nepochs_ILD,scale_factor=scale_factor_test,load=load,
        skip_train_mse=skip_train_mse,skip_train_ILD=skip_train_ILD,nsamples=nsamples,batch_size=batch_size, batch_memory=batch_memory,
        time_context=time_context, overlap=overlap,nprocs=nprocs,mult_factor_in=scale_factor,mult_factor_out=scale_factor,skip_sep=skip_sep, mix_type=mix_type)
    f = file(os.path.join(output,"models","loss_"+model_name+".data"), 'wb')
