...
def filterSpec(mag,pitches,interval=50,tuning_freq=440,nharmonics=20,fmin=55,fmax=14000,iscale='lin',sampleRate=44100):
    filtered = np.zeros_like(mag)
    for p in range(len(pitches)): #for each pitch contour
        #filter the spectrogram according to the f0 and the corresponding partials
        #slices = [self.getBands(pitches[p,t],mag.shape[-1]) for t in range(len(pitches[p]))]
        #import pdb;pdb.set_trace()
        for t in range(len(pitches[p])):
            if pitches[p,t] > 0:
                slices_y = util.slicefft(pitches[p,t],size=(mag.shape[-1]-1)*2,interval=interval,tuning_freq=tuning_freq,nharmonics=nharmonics,fmin=fmin,fmax=fmax,iscale=iscale,sampleRate=sampleRate)
                filtered[t,slices_y] = 1.
    return filtered
...
def build_ca(input_var1=None, input_var2=None, batch_size=32,time_context=30,feat_size=1025):

    input_shape=(batch_size,1,time_context,feat_size)
    #scaled_tanh = lasagne.nonlinearities.ScaledTanH(scale_in=1, scale_out=0.5)

    l_in_1 = lasagne.layers.InputLayer(shape=input_shape, input_var=input_var1)
    l_in_2 = lasagne.layers.InputLayer(shape=input_shape, input_var=input_var2)
    ...
    l_conv12 = lasagne.layers.Conv2DLayer(l_in_2, num_filters=30, filter_size=(1,30),stride=(1,3), pad='valid', nonlinearity=None)
    ...
    l_fc111=lasagne.layers.DenseLayer(l_fc,l_conv21.output_shape[1]*l_conv21.output_shape[2]*l_conv21.output_shape[3])
    l_reshape11 = lasagne.layers.ReshapeLayer(l_fc111,(batch_size,l_conv21.output_shape[1],l_conv21.output_shape[2], l_conv21.output_shape[3]))
    l_inverse111=lasagne.layers.InverseLayer(l_reshape11, l_conv21)
    l_inverse311=lasagne.layers.InverseLayer(l_inverse111, l_pool11)
    l_inverse411=lasagne.layers.InverseLayer(l_inverse311, l_conv11)

    l_fc112=lasagne.layers.DenseLayer(l_fc,l_conv21.output_shape[1]*l_conv21.output_shape[2]*l_conv21.output_shape[3])
    l_reshape12 = lasagne.layers.ReshapeLayer(l_fc111,(batch_size,l_conv21.output_shape[1],l_conv21.output_shape[2], l_conv21.output_shape[3]))
    l_inverse112=lasagne.layers.InverseLayer(l_reshape12, l_conv21)
    l_inverse312=lasagne.layers.InverseLayer(l_inverse112, l_pool11)
    l_inverse412=lasagne.layers.InverseLayer(l_inverse312, l_conv11)


  
    l_fc121=lasagne.layers.DenseLayer(l_fc,l_conv22.output_shape[1]*l_conv22.output_shape[2]*l_conv22.output_shape[3])
    l_reshape21 = lasagne.layers.ReshapeLayer(l_fc121,(batch_size,l_conv22.output_shape[1],l_conv22.output_shape[2], l_conv22.output_shape[3]))
    l_inverse121=lasagne.layers.InverseLayer(l_reshape21, l_conv22)
    l_inverse321=lasagne.layers.InverseLayer(l_inverse121, l_pool12)
    l_inverse421=lasagne.layers.InverseLayer(l_inverse321, l_conv12)

    l_fc122=lasagne.layers.DenseLayer(l_fc,l_conv22.output_shape[1]*l_conv22.output_shape[2]*l_conv22.output_shape[3])
    l_reshape22 = lasagne.layers.ReshapeLayer(l_fc122,(batch_size,l_conv22.output_shape[1],l_conv22.output_shape[2], l_conv22.output_shape[3]))
    l_inverse122=lasagne.layers.InverseLayer(l_reshape22, l_conv22)
    l_inverse322=lasagne.layers.InverseLayer(l_inverse122, l_pool12)
    l_inverse422=lasagne.layers.InverseLayer(l_inverse322, l_conv12)


    l_merge=lasagne.layers.ConcatLayer([l_inverse411,l_inverse421,l_inverse412,l_inverse422],axis=1)
...
def train_auto(train,fun,transform,testdir,outdir,num_epochs=30,model="1.pkl",scale_factor=0.3,load=False,skip_train=False,skip_sep=False):
    ...
    network2 = fun(input_var1=input_var1,input_var2=input_var2,batch_size=train.batch_size,time_context=train.time_context,feat_size=train.input_size)
    ...
    voc=input_mask2 * prediction2[:,0:1,:,:] + (1.-input_mask2) * prediction2[:,2:3,:,:]
    acco=input_mask2 * prediction2[:,1:2,:,:] + (1.-input_mask2) * prediction2[:,3:4,:,:]

    # voc=prediction2[:,0:1,:,:]
    # acco=prediction2[:,1:2,:,:]

    mask1=voc/(voc+acco+eps*rand_num)
    mask2=acco/(voc+acco+eps*rand_num)

    vocals=mask1*(input_var1[:,0:1,:,:]+input_var2[:,0:1,:,:])
    acc=mask2*(input_var1[:,0:1,:,:]+input_var2[:,0:1,:,:])

    target_vocals = target_var2[:,0:1,:,:]
    target_acc = target_var2[:,1:2,:,:]


    train_loss_recon_vocals = lasagne.objectives.squared_error(vocals,target_vocals)
    train_loss_recon_acc = alpha * lasagne.objectives.squared_error(acc,target_acc)    
    train_loss_recon_neg_voc = beta_voc * lasagne.objectives.squared_error(vocals,target_acc)
    train_loss_recon_neg_acc = beta_acc * lasagne.objectives.squared_error(acc,target_vocals)
    ...
    loss=abs(vocals_error+acc_error-negative_error_voc-negative_error_acc)

    params1 = lasagne.layers.get_all_params(network2, trainable=True)

    updates = lasagne.updates.adadelta(loss, params1)

    train_fn = theano.function([input_var1,input_var2,target_var2,input_mask2], loss, updates=updates,allow_input_downcast=True)

    train_fn1 = theano.function([input_var1,input_var2,target_var2,input_mask2], [vocals_error,acc_error,negative_error_voc,negative_error_acc], allow_input_downcast=True)

    predict_function2=theano.function([input_var1,input_var2,input_mask2],[vocals,acc],allow_input_downcast=True)
    ...
    if not skip_train:
        ...
        for epoch in range(num_epochs):
            ...
            for batch in range(train.iteration_size):
                ...
                masks = masks + 1e-18 * np.random.random(masks.shape)                  
               
                # targets[:,0,:,:]=target[:,:,:jump]
                # targets[:,1,:,:]=target[:,:,jump:jump*2]
                inputs1 = inputs * masks + 1e-18 * np.random.random(masks.shape)
                inputs2 = inputs * (1.-masks) + 1e-18 * np.random.random(masks.shape)
                inputs=None
                targets[:,0,:,:]=target[:,:,:jump] 
                targets[:,1,:,:]=target[:,:,jump:jump*2] 
                target=None
                ...
                train_err+=train_fn(inputs1,inputs2,targets,masks)
                [vocals_erre,acc_erre,betae_voc,betae_acc]=train_fn1(inputs1,inputs2,targets,masks)
    ...
    if not skip_sep:
        ...
        #compute transform
        for f in os.listdir(testdir+"Wavfile/"):
            if f.endswith(".wav"):
                loader = essentia.standard.AudioLoader(filename=testdir+"Wavfile/"+f)
                audioObj = loader()
                ...
                lines = np.loadtxt(testdir+"PitchLabel/"+f.replace('wav','pv'), comments="#", delimiter="\n", unpack=False)

                mask=filterSpec(mag,lines[np.newaxis,:],interval=train.interval,tuning_freq=train.tuning_freq,nharmonics=train.nharmonics,fmin=train.fmin,fmax=train.fmax,iscale=train.iscale,sampleRate=train.sampleRate)
                ...
                batches_mask,nchunks_masks = util.generate_overlapadd(mask,input_size=mag.shape[-1],time_context=train.time_context,overlap=train.overlap,batch_size=train.batch_size,sampleRate=44100)
                output=[]
                #output1=[]

                batch_no=1
                for nbatch in range(len(batches)):
                    ...
                    # lasagne.layers.set_all_param_values(network2,params)
                    start_time=time.time()
                    output.append(predict_function2(batches[nbatch]*batches_mask[nbatch],batches[nbatch]*(1.-batches_mask[nbatch]),batches_mask[nbatch]))
...
if __name__ == "__main__": 
    ...
    ld1 = LargeDatasetPitch1(path_transform_in=dirtt[id_transform], batch_size=batch_size, batch_memory=batch_memory, time_context=time_context, overlap=overlap, nprocs=nprocs,mult_factor_in=scale_factor,mult_factor_out=scale_factor,
        sampleRate=tt[id_transform].sampleRate,fmin=tt[id_transform].fmin, fmax=tt[id_transform].fmax,ttype=tt[id_transform].ttype,iscale=tt[id_transform].iscale, 
    ...
    train_errs=train_auto(train=ld1,fun=build_ca,transform=tt[id_transform],outdir=db+'output/'+model+"/",testdir=db,model=db+"models/"+"model_"+model+".pkl",num_epochs=nepochs,scale_factor=scale_factor)   #,load=True,skip_train=True,skip_sep=True      
