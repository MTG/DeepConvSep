...
def train_auto(train,fun,transform,testdir,outdir,num_epochs=30,model="1.pkl",scale_factor=0.3,load=False,skip_train=False,skip_sep=False):
    ...
    # voc1=input_mask2 * prediction2[:,0:1,:,:]+eps*rand_num
    # acco1=input_mask2 * prediction2[:,1:2,:,:]+eps*rand_num
    # voc2=(1.-input_mask2) * prediction2[:,2:3,:,:]+eps*rand_num
    # acco2=(1.-input_mask2) * prediction2[:,3:4,:,:]+eps*rand_num
    # voc = voc1 + voc2
    # acco = acco1 + acco2
    ...
    if not skip_train:
        ...
        for epoch in range(num_epochs):
            ...
            for batch in range(train.iteration_size):
                ...
                inputs = inputs * masks + 1e-18 * np.random.random(masks.shape)
                targets[:,0,:,:]=target[:,:,:jump] * masks[:,0,:,:] 
                targets[:,1,:,:]=target[:,:,jump:jump*2] * masks[:,0,:,:] 
                target=None
                #import pdb;pdb.set_trace()
    ...
    if not skip_sep:
        ...
        for f in os.listdir(testdir):
            if f.endswith(".wav"):
                ...
                mask=filterSpec(mag,lines[np.newaxis,np.newaxis,:],interval=train.interval,tuning_freq=train.tuning_freq,nharmonics=train.nharmonics,fmin=train.fmin,fmax=train.fmax,iscale=train.iscale,sampleRate=train.sampleRate)
                ...
                batches_mask,nchunks_masks = util.generate_overlapadd(mag,input_size=mag.shape[-1],time_context=train.time_context,overlap=train.overlap,batch_size=train.batch_size,sampleRate=44100)
                output=[]
                #output1=[]
                ...
                for nbatch in range(len(batches)):
                    ...
                    # lasagne.layers.set_all_param_values(network2,params)
                    start_time=time.time()
                    output.append(predict_function2(batches[nbatch]))
...
if __name__ == "__main__": 
    ...
    train_errs=train_auto(train=ld1,fun=build_ca,transform=tt[id_transform],outdir=db+'output/'+model+"/",testdir=db,model=db+"models/"+"model_"+model+".pkl",num_epochs=nepochs,scale_factor=scale_factor,skip_sep=True )   #,load=True,skip_train=True     
