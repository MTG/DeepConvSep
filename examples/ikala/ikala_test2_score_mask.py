...
def build_ca(input_var=None, input_mask=None, batch_size=32,time_context=30,feat_size=1025):
    ...
    l_in_mask = lasagne.layers.InputLayer(shape=input_shape, input_var=input_mask)
    ...
    l_conv1m = lasagne.layers.Conv2DLayer(l_in_mask, num_filters=30, filter_size=(1,30),stride=(1,3), pad='valid', nonlinearity=None)
    l_conv1bm= lasagne.layers.BiasLayer(l_conv1m)

    l_pool1m = lasagne.layers.MaxPool2DLayer(l_conv1bm, pool_size=(1, 4))

    l_conv2m = lasagne.layers.Conv2DLayer(l_pool1m, num_filters=30, filter_size=(10,20),stride=(1,1), pad='valid', nonlinearity=None)
    l_conv2bm= lasagne.layers.BiasLayer(l_conv2m)

    l_merge = lasagne.layers.ElemwiseMergeLayer([l_conv2, l_conv2m], T.mul)


    l_fc=lasagne.layers.DenseLayer(l_merge,256)
...
def train_auto(train,fun,transform,testdir,outdir,num_epochs=30,model="1.pkl",scale_factor=0.3,load=False,skip_train=False,skip_sep=False):
    ...
    network2 = fun(input_var=input_var2,input_mask=input_mask2,batch_size=train.batch_size,time_context=train.time_context,feat_size=train.input_size)
    ...
    train_fn = theano.function([input_var2,target_var2,input_mask2], loss, updates=updates,allow_input_downcast=True)

    train_fn1 = theano.function([input_var2,target_var2,input_mask2], [vocals_error,acc_error,negative_error_voc,negative_error_acc], allow_input_downcast=True)

    predict_function2=theano.function([input_var2,input_mask2],[vocals,acc],allow_input_downcast=True)
    ...
    if not skip_train:
        ...
        for epoch in range(num_epochs):
            ...
            for batch in range(train.iteration_size):
                ...
                masks = masks + 1e-9 * np.random.random(masks.shape)                  
               
                targets[:,0,:,:]=target[:,:,:jump]
                targets[:,1,:,:]=target[:,:,jump:jump*2]
                #inputs = inputs * masks + 1e-9 * np.random.random(masks.shape)
                #targets[:,0,:,:]=target[:,:,:jump] * masks[:,0,:,:] + 1e-9 * np.random.random(masks[:,0,:,:].shape)
                #targets[:,1,:,:]=target[:,:,jump:jump*2] * masks[:,0,:,:] + 1e-9 * np.random.random(masks[:,0,:,:].shape)       
                target=None
        
                train_err+=train_fn(inputs,targets,masks)
                [vocals_erre,acc_erre,betae_voc,betae_acc]=train_fn1(inputs,targets,masks)
        ...
        for f in os.listdir(testdir+"Wavfile/"):
            if f.endswith(".wav"):
                ...
                for nbatch in range(len(batches)):
                    ...
                    # lasagne.layers.set_all_param_values(network2,params)
                    start_time=time.time()
                    output.append(predict_function2(batches[nbatch],batches_mask[nbatch]))
...
if __name__ == "__main__": 
    ...
    train_errs=train_auto(train=ld1,fun=build_ca,transform=tt[id_transform],outdir=db+'output/'+model+"/",testdir=db,model=db+"models/"+"model_"+model+".pkl",num_epochs=nepochs,scale_factor=scale_factor,load=True,skip_train=True  )   #,load=True,skip_train=True     
