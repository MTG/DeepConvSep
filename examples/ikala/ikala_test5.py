...
def build_ca(input_var=None, batch_size=32,time_context=30,feat_size=1025):
    ...
    l_fc=lasagne.layers.DenseLayer(l_merge1,time_context*15)
    ...
    l_pitch1=lasagne.layers.DenseLayer(l_fc,int(l_fc.output_shape[-1]/15),nonlinearity=None)
    #print 'bottleneck input:', l_pitch1.output_shape
    l_pitch = lasagne.layers.ReshapeLayer(l_pitch1,(batch_size,time_context,1))

    l_voiced1=lasagne.layers.DenseLayer(l_fc,int(l_fc.output_shape[-1]/15),nonlinearity=None)
    #print 'bottleneck input:', l_pitch1.output_shape
    l_voiced = lasagne.layers.ReshapeLayer(l_voiced1,(batch_size,time_context,1))
   
    return l_out,l_pitch,l_voiced
...
def train_auto(train,fun,transform,testdir,outdir,num_epochs=30,model="1.pkl",scale_factor=0.3,load=False,skip_train=False,skip_sep=False):
    ...
    target_pitch = T.tensor3('target_pitch')
    target_voiced = T.tensor3('target_voiced')
    ...
    network2,networkp,networkv = fun(input_var=input_var2,batch_size=train.batch_size,time_context=train.time_context,feat_size=train.input_size)
    
    if load:
        params=load_model(model)
        lasagne.layers.set_all_param_values(network2,params)
        #TOIMPLEMENT
        #lasagne.layers.set_all_param_values(networkp,params)
        #lasagne.layers.set_all_param_values(networkv,params)

    prediction2 = lasagne.layers.get_output(network2, deterministic=True)
    predictionp = lasagne.layers.get_output(networkp, deterministic=True) 
    predictionv = lasagne.layers.get_output(networkv, deterministic=True) 


    train_loss_pitch = palpha * lasagne.objectives.squared_error(target_voiced*predictionp,target_voiced*target_pitch)
    pitch_error = train_loss_pitch.sum()
    train_loss_voiced = valpha * lasagne.objectives.squared_error(predictionv,target_voiced)
    voiced_error = train_loss_voiced.sum()
    ...
    loss=abs(voiced_error + pitch_error + vocals_error + acc_error - negative_error_voc)

    params1 = lasagne.layers.get_all_params(network2, trainable=True)

    updates = lasagne.updates.adadelta(loss, params1)

    train_fn = theano.function([input_var2,target_var2,target_pitch,target_voiced], loss, updates=updates,allow_input_downcast=True)
    ...
    train_fnp = theano.function([input_var2,target_pitch,target_voiced], pitch_error, allow_input_downcast=True)
    train_fnv = theano.function([input_var2,target_voiced], voiced_error, allow_input_downcast=True)
    ...
    if not skip_train:
        ...
        for epoch in range(num_epochs):
            ...
            #for batch in range(2):
                ...
                target=None
                train_err+=train_fn(inputs,targets,pitches,voiced)
                [vocals_erre,acc_erre,betae_voc,betae_acc]=train_fn1(inputs,targets)
                pitch_erre=train_fnp(inputs,pitches,voiced)
                voiced_erre=train_fnv(inputs,voiced)
                vocals_err += vocals_erre
                acc_err += acc_erre           
                beta_voc+= betae_voc
                beta_acc+= betae_acc
                pitch_err += pitch_erre
                voiced_err += voiced_erre
                train_batches += 1
            ...
            logging.info("  training loss for pitch:\t\t{:.6f}".format(pitch_err/train_batches))
            logging.info("  training loss for voiced:\t\t{:.6f}".format(voiced_err/train_batches))
