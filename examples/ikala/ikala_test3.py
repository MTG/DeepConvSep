...
def build_ca_lstm1(input_var=None, batch_size=32,time_context=30,feat_size=102,num_units=10):
    ...
    l_conv1 = lasagne.layers.Conv2DLayer(l_in_1, num_filters=30, filter_size=(1,30),stride=(1,3), pad='valid', nonlinearity=None)
    l_conv1b= lasagne.layers.BiasLayer(l_conv1)
    #print 'CNN1 output:', l_conv1b.output_shape
    ...
    #print 'CNN2 output:', l_conv2b.output_shape
    l_ds1 = lasagne.layers.DimshuffleLayer(l_conv2b, (0,2,1,3))
    #print 'Dims output:', l_ds1.output_shape
    #l_fl1 = lasagne.layers.FlattenLayer(l_conv2b)
    #l_rs1 = lasagne.layers.ReshapeLayer(l_conv2b, (-1, [2], [3]))
    l_rs1 = lasagne.layers.ReshapeLayer(l_ds1, ([0], [1], -1))

    #print 'LSTM input:', l_rs1.output_shape
    #lasagne.layers.dropout(network, p=.5),  # Dropout
    l_lstm1 = lasagne.layers.LSTMLayer(l_rs1,num_units=num_units,nonlinearity=lasagne.nonlinearities.tanh)
    #print 'LSTM output:', l_lstm1.output_shape

    l_fc = ReshapeLayer(l_lstm1, (batch_size,-1))

    # l_fc=lasagne.layers.DenseLayer(l_shp,256)
    # l_fc=lasagne.layers.DenseLayer(l_conv2b,256)
    # print 'bottleneck input:', l_fc.input_shape
    # print 'bottleneck output:', l_fc.output_shape
...
def build_ca_lstm(input_var=None, batch_size=32,time_context=30,feat_size=102,num_units=10):
    ...
    l_conv1 = lasagne.layers.Conv2DLayer(l_in_1, num_filters=30, filter_size=(1,50),stride=(1,5), pad='valid', nonlinearity=None)
    l_conv1b= lasagne.layers.BiasLayer(l_conv1)
    #print 'CNN1 output:', l_conv1b.output_shape
    ...
    #print 'CNN2 output:', l_conv2b.output_shape
    #l_ds1 = lasagne.layers.DimshuffleLayer(l_pool1, (0,2,1,3))
    #print 'Dims output:', l_ds1.output_shape
    #l_fl1 = lasagne.layers.FlattenLayer(l_conv2b)
    #l_rs1 = lasagne.layers.ReshapeLayer(l_conv2b, (-1, [2], [3]))
    l_rs1 = lasagne.layers.ReshapeLayer(l_pool1, ([0], [1], -1))

    #print 'LSTM input:', l_rs1.output_shape
    #lasagne.layers.dropout(network, p=.5),  # Dropout
    l_lstm1 = lasagne.layers.LSTMLayer(l_rs1,num_units=num_units,nonlinearity=lasagne.nonlinearities.rectify)
    #print 'LSTM output:', l_lstm1.output_shape
    ...
    l_fc11=lasagne.layers.DenseLayer(l_fc,l_pool1.output_shape[1]*l_pool1.output_shape[2]*l_pool1.output_shape[3])
    l_reshape1 = lasagne.layers.ReshapeLayer(l_fc11,(batch_size,l_pool1.output_shape[1],l_pool1.output_shape[2], l_pool1.output_shape[3]))
    l_inverse31=lasagne.layers.InverseLayer(l_reshape1, l_pool1)
    l_inverse41=lasagne.layers.InverseLayer(l_inverse31, l_conv1)

  
    l_fc12=lasagne.layers.DenseLayer(l_fc,l_pool1.output_shape[1]*l_pool1.output_shape[2]*l_pool1.output_shape[3])
    l_reshape2 = lasagne.layers.ReshapeLayer(l_fc12,(batch_size,l_pool1.output_shape[1],l_pool1.output_shape[2], l_pool1.output_shape[3]))
    l_inverse32=lasagne.layers.InverseLayer(l_reshape2, l_pool1)
    l_inverse42=lasagne.layers.InverseLayer(l_inverse32, l_conv1)
...
def build_ca_lstm_split(input_var=None, batch_size=32,time_context=30,feat_size=102,num_units=10):
    ...
    l_ds11 = lasagne.layers.DimshuffleLayer(l_pool11, (0,2,1,3))
    l_rs11 = lasagne.layers.ReshapeLayer(l_ds11, ([0], [1], -1))
    ...
    l_ds12 = lasagne.layers.DimshuffleLayer(l_pool12, (0,2,1,3))
    l_rs12 = lasagne.layers.ReshapeLayer(l_ds12, ([0], [1], -1))

    l_merge1=lasagne.layers.ConcatLayer([l_rs11,l_rs12],axis=1)


    #print 'LSTM input:', l_rs1.output_shape
    #lasagne.layers.dropout(network, p=.5),  # Dropout
    l_lstm1 = lasagne.layers.LSTMLayer(l_merge1,num_units=num_units,nonlinearity=lasagne.nonlinearities.rectify)
    #print 'LSTM output:', l_lstm1.output_shape
    ...
    l_fc11=lasagne.layers.DenseLayer(l_fc,l_pool11.output_shape[1]*l_pool11.output_shape[2]*l_pool11.output_shape[3])
    l_reshape1 = lasagne.layers.ReshapeLayer(l_fc11,(batch_size,l_pool11.output_shape[1],l_pool11.output_shape[2], l_pool11.output_shape[3]))
    l_inverse31=lasagne.layers.InverseLayer(l_reshape1, l_pool11)
    l_inverse41=lasagne.layers.InverseLayer(l_inverse31, l_conv11)

  
    l_fc12=lasagne.layers.DenseLayer(l_fc,l_pool12.output_shape[1]*l_pool12.output_shape[2]*l_pool12.output_shape[3])
    l_reshape2 = lasagne.layers.ReshapeLayer(l_fc12,(batch_size,l_pool12.output_shape[1],l_pool12.output_shape[2], l_pool12.output_shape[3]))
    l_inverse32=lasagne.layers.InverseLayer(l_reshape2, l_pool12)
...
if __name__ == "__main__": 
    ...
    train_errs=train_auto(train=ld1,fun=build_ca_lstm,transform=tt[id_transform],outdir=db+'output/'+model+"/",testdir=db+'Wavfile/',model=db+"models/"+"model_"+model+".pkl",num_epochs=nepochs,scale_factor=scale_factor)   #,load=True,skip_train=True     
