...
def build_ca(input_var=None, input_pitch=None, batch_size=32,time_context=30,feat_size=1025,num_units=10):

    input_shape=(batch_size,1,time_context,feat_size)
    input_shape_pitch=(batch_size,time_context,1)
    #scaled_tanh = lasagne.nonlinearities.ScaledTanH(scale_in=1, scale_out=0.5)

    l_in_pitch = lasagne.layers.InputLayer(shape=input_shape_pitch, input_var=input_pitch)
    print 'pitch output:', l_in_pitch.output_shape
    ...
    print 'CNN2 output:', l_pool1.output_shape
    ...
    l_merge_lstm=lasagne.layers.ConcatLayer([l_lstm1,l_in_pitch],axis=2)
    print 'Concat output:', l_merge_lstm.output_shape

    l_fc = ReshapeLayer(l_merge_lstm, (batch_size,-1))
...
def train_auto(train,fun,transform,testdir,outdir,num_epochs=30,model="1.pkl",scale_factor=0.3,load=False,skip_train=False,skip_sep=False):
    ...
    network2= fun(input_var=input_var2,input_pitch=pitch_var2,batch_size=train.batch_size,time_context=train.time_context,feat_size=train.input_size)
    ...
    train_fn = theano.function([input_var2,target_var2,pitch_var2], loss, updates=updates,allow_input_downcast=True)

    train_fn1 = theano.function([input_var2,target_var2,pitch_var2], [vocals_error,acc_error,negative_error_voc,negative_error_acc], allow_input_downcast=True)
 
    predict_function2=theano.function([input_var2,pitch_var2],[vocals,acc],allow_input_downcast=True)
    predict_function3=theano.function([input_var2,pitch_var2],[prediction2[:,0:1,:,:],prediction2[:,1:2,:,:]],allow_input_downcast=True)
    ...
    if not skip_train:
        ...
        for epoch in range(num_epochs):
            ...
            for batch in range(train.iteration_size): 
                inputs, target, pitches = train()
                #voiced = (pitches==0).astype(float)
                ...
                #pitches=np.reshape(pitch,(pitch.shape[0],1,pitch.shape[1],pitch.shape[2]))       
                
                targets[:,0,:,:]=target[:,:,:jump]
                targets[:,1,:,:]=target[:,:,jump:jump*2]         
                target=None
                train_err+=train_fn(inputs,targets,pitches)
                [vocals_erre,acc_erre,betae_voc,betae_acc]=train_fn1(inputs,targets,pitches)
...
if __name__ == "__main__": 
    ...
    ld1 = LargeDatasetPitch1(path_transform_in=dirtt[id_transform], batch_size=batch_size, batch_memory=batch_memory, \
        time_context=time_context, overlap=overlap, nprocs=nprocs,mult_factor_in=scale_factor,mult_factor_out=scale_factor,\
        ...
        nharmonics=20, interval=50, tuning_freq=440, pitched=True, pitch_norm=127.)
