...
def train_auto(train,fun,transform,testdir,outdir,num_epochs=30,model="1.pkl",scale_factor=0.3,load=False,skip_train=False,skip_sep=False):
    ...
    voc=prediction2[:,0:1,:,:] + prediction2[:,2:3,:,:]
    acco=prediction2[:,1:2,:,:] + prediction2[:,3:4,:,:]
    ...
    train_fn = theano.function([input_var1,input_var2,target_var2], loss, updates=updates,allow_input_downcast=True)

    train_fn1 = theano.function([input_var1,input_var2,target_var2], [vocals_error,acc_error,negative_error_voc,negative_error_acc], allow_input_downcast=True)

    predict_function2=theano.function([input_var1,input_var2],[vocals,acc],allow_input_downcast=True)
    ...
    if not skip_train:
        ...
        for epoch in range(num_epochs):
            ...
            for batch in range(train.iteration_size):
                ...
                [vocals_erre,acc_erre,betae_voc,betae_acc]=train_fn1(inputs1,inputs2,targets)
        ...
        for f in os.listdir(testdir):
            if f.endswith(".wav"):
                ...
                for nbatch in range(len(batches)):
                    ...
                    # lasagne.layers.set_all_param_values(network2,params)
                    start_time=time.time()
                    output.append(predict_function2(batches[nbatch]*batches_mask[nbatch],batches[nbatch]*(1.-batches_mask[nbatch])))
...
if __name__ == "__main__": 
    if len(sys.argv)>-1:
        ...
        if kwargs.__getattribute__('model'):
            ...
            model="fft_1024_mask_double_var1"
    ...
    train_errs=train_auto(train=ld1,fun=build_ca,transform=tt[id_transform],outdir=db+'output/'+model+"/",testdir=db,model=db+"models/"+"model_"+model+".pkl",num_epochs=nepochs,scale_factor=scale_factor )   #,load=True,skip_train=True     
