...
if __name__ == "__main__": 
    ...
    if kwargs.__getattribute__('feature_path'):
        ...
        feature_path=os.path.join(db,'transforms','t1_mix_aug') 
    assert os.path.isdir(db), "Please input the directory for the dataset with --db path"
    
    # Compute the features of the songs without permutations

    mixture_directory=os.path.join(db,'Mixtures')
    source_directory=os.path.join(db,'Sources')

    tt = None
    dirlist = sorted(os.listdir(os.path.join(mixture_directory,"Dev")))
    for f in sorted(dirlist):
        if not f.startswith('.'):
            ...
            util.writeAudioScipy(os.path.join(mixOutDir,'mixture.wav'),mix_raw,sampleRate,bitrate)
    ...
    # Clean the dirlist before iterating over the list
    for k in range(len(dirlist)):
        if dirlist[k].startswith('.'):
            dirlist[k] = None

    offsets = np.arange(4)
    comb = 0

    # Calculate a random number from 0 to 9
    batch = np.random.randint(10, size=1)
    for f in it.combinations(dirlist,4):
        if comb % 10 == batch[0]:
            print("\033[1;34m" + "\nProcessing batch of files:" + "\033[0;0m")
            print f
            # Calculate a random number from 0 to 9
            n = np.random.randint(10, size=1)
            perm = 0
            for p in it.permutations(offsets):
                if perm % 10 == n[0]:
                    print("\033[1;34m" +"- Processing permutation: %d" %perm + " of combination %d" %comb + "\033[0;0m")

                    # Read the source audio files
                    bass_folder = f[p[0]]
                    bass, sampleRate, bitrate = util.readAudioScipy(os.path.join(source_directory,"Dev",bass_folder,"bass.wav"))
                    if bass.shape[1]>1:
                        bass[:,0] = (bass[:,0] + bass[:,1]) / 2
                        bass = bass[:,0]

                    drums_folder = f[p[1]]
                    drums, sampleRate, bitrate = util.readAudioScipy(os.path.join(source_directory,"Dev",drums_folder,"drums.wav"))
                    if drums.shape[1]>1:
                        drums[:,0] = (drums[:,0] + drums[:,1]) / 2
                        drums = drums[:,0]

                    others_folder = f[p[2]]
                    others, sampleRate, bitrate = util.readAudioScipy(os.path.join(source_directory,"Dev",others_folder,"other.wav"))
                    if others.shape[1]>1:
                        others[:,0] = (others[:,0] + others[:,1]) / 2
                        others = others[:,0]

                    vocals_folder = f[p[3]]
                    vocals, sampleRate, bitrate = util.readAudioScipy(os.path.join(source_directory,"Dev",vocals_folder,"vocals.wav"))
                    if vocals.shape[1]>1:
                        vocals[:,0] = (vocals[:,0] + vocals[:,1]) / 2
                        vocals = vocals[:,0]
                    
                    # Get the minimum length
                    lengths = [len(bass), len(drums), len(others), len(vocals)]
                    min_length = min(lengths)

                    # Generate the current variation of the mixture
                    mix_raw = np.zeros(min_length)
                    mix_raw = (bass[0:min_length] + drums[0:min_length] + others[0:min_length] + vocals[0:min_length])/4
                    ...
                    for i in range(number_of_blocks):
                        ...
                        if not os.path.exists(feature_path):
                            os.makedirs(feature_path)
                        tt.compute_transform(audio,os.path.join(feature_path,"combination_"+str(comb)+"_perm_"+str(p[0])+"_"+str(p[1])+"_"+str(p[2])+"_"+str(p[3])+"_block_"+str(i+1)+'.data'),phase=False)
                        audio = None
        ...
        # Increment the combination number
        comb=comb+1