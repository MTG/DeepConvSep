...
def expandMidi(instrument,FilePath,beginTime,finishTime,interval,tuning_freq,nharmonics,samplerate,hop,window,timeSpan_on,timeSpan_off,nframes,fermata=0.):
    ...
    midifile = os.path.join(FilePath,instrument)
...
def train_auto(filein,outdir,model,scale_factor=0.3,time_context = 30,overlap = 20,batch_size=32,input_size=2049, frameSize=4096, hopSize=512):
    ...
    sources_midi = ['bassoon_b.txt','clarinet_b.txt','saxophone_b.txt','violin_b.txt']
    ...
    eps=1e-18
    network2 = build_ca(input_var2,batch_size,time_context,input_size,4)
    ...
    nframes = int(np.ceil(len(audioObj) / np.double(hopSize))) + 2
    ...
    if sampleRate == 44100:
        ...
        for i in range(len(sources)):
            ng =getMidiNum(sources_midi[i],os.path.dirname(filein),0,40.0)
            nelem_g = np.maximum(ng,nelem_g)
        melody = np.zeros((len(sources),int(nelem_g),2*nharmonics+3))
        for i in range(len(sources)):
            tmp = expandMidi(sources_midi[i],os.path.dirname(filein),0,40.0,interval,tuning_freq,nharmonics,sampleRate,hopSize,frameSize,0.2,0.2,nframes,0.5)
            melody[i,:tmp.shape[0],:] = tmp
            tmp = None

        jump = mag.shape[-1]

        masks_temp = filterSpec(mag,melody,len(sources),0,nframes)
        masks = np.ones((len(sources),mag.shape[0],mag.shape[1]))
        ...
        batches,nchunks = util.generate_overlapadd(masks,input_size=masks.shape[-1],time_context=time_context,overlap=toverlap,batch_size=batch_size,sampleRate=44100)
        masks = None
